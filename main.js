// Verificar se os eventos estão sendo atribuídos corretamente
if (cartIcon) {
    cartIcon.onclick = () => {
...
function validateRegisterForm(event) {
    ...
    if (!telefoneRegex.test(telefone)) {
        ...
        showAlert('error', 'Oops...', 'O número de telefone deve ter 9 dígitos e começar com 9.');
        return; // Não continua com o envio do formulário
    }

    if (!nomeApelidoRegex.test(nome) || !nomeApelidoRegex.test(apelido)) {
        showAlert('error', 'Oops...', 'O nome e o apelido não devem conter números ou caracteres especiais.');
        return; // Não continua com o envio do formulário
    }
...
function addCartClicked(event) {
    if (!isLoggedIn()) {
        showAlert('info', 'Iniciar Sessão', 'Por favor, inicie sessão para adicionar produtos ao carrinho.');
        return;
    }

    var titulo = event.currentTarget.getAttribute('data-titulo');
    var preco = event.currentTarget.getAttribute('data-preco');
    var caminho_imagem = event.currentTarget.getAttribute('data-caminho-imagem');
...
function addProductToCart(title, price, productImg) {
    ...
    for (var i = 0; i < cartItemsTitles.length; i++) {
        if (cartItemsTitles[i] && cartItemsTitles[i].innerText && cartItemsTitles[i].innerText.trim() === title.trim()) {
            isDuplicate = true;
            break;
        }
    }
    ...
    if (isDuplicate) {
        showAlert('info', 'Livro Já Adicionado', 'Você já adicionou este livro ao seu carrinho.');
        return;
    }
    ...
    showAlert('info', 'Livro Adicionado', 'Você adicionou um livro ao seu carrinho.');
}
...
function addClickEventToBooks() {
    ...
    bookImages.forEach(image => {
        image.addEventListener('click', function () {
            ...
            const price = this.nextElementSibling.nextElementSibling.nextElementSibling.textContent;
